...
public class CrosshairMod extends HudMod {
	...
	private static final ResourceLocation CLIENT_CROSSHAIRS = new ResourceLocation(
			"textures/gui" + "/sol_client_crosshairs.png");
	...
	public void onCrosshairRender(PreGameOverlayRenderEvent event) {
		if (event.type == GameOverlayElement.CROSSHAIRS) {
			...
			if ((!debug && mc.gameSettings.showDebugInfo)
					|| (!spectatorAlways && (mc.playerController.getCurrentGameType() == GameType.SPECTATOR
			...
			GlStateManager.translate((int) (resolution.getScaledWidth() / getScale() / 2 - 7),
					(int) (resolution.getScaledHeight() / getScale() / 2 - 7), 0);
			...
			if (highlightEntities && mc.objectMouseOver != null && mc.objectMouseOver.entityHit != null
					&& !(mc.objectMouseOver.entityHit.isInvisible()
